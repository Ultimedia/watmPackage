<?php
	...
	$dbc = getDBConnection();		
	$sql = "SELECT user_id, stamina_score, strength_score, equipment_score FROM watm_users WHERE facebook_id = ?";
	if($stmt = $dbc->prepare($sql))
	{
		...
		{
			$stmt->store_result();
			$stmt->bind_result($userid, $stamina_score, $strength_score, $equipment_score);
			$stmt->fetch();
